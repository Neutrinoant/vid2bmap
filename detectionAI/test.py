...
from model import SiameseNetwork, SiameseNetworkColor
...
def build_label_json_color(ckpt_path, outpath=""):
    Config.seed_everything(100)
    ...
    # ready for test dataset
    folder_dataset_train = dset.ImageFolder(root=Config.training_dir)
    train_dataset = LabeledDataset(imageFolderDataset=folder_dataset_train,     # alphabetical order
                                            transform=transform,
                                            should_invert=should_invert,
                                            gray=gray)
    ...
    with torch.no_grad():   # gradient 계산 메모리 사용을 꺼버림
        ...
        # 라벨별로 핵심지표 계산
        L_uid = torch.unique(L_arr)     # alphabetical order
    ...
    if outpath != "":
        ...
        jsondata = {
            "ids": labels.tolist(), 
            "names": folder_dataset_train.classes,      # alphabetical order
            "centroids_y": centroids_y.tolist()
        }
        with open(outpath, "wt") as f:
            json.dump(jsondata, f)
...
def test(ckpt_path, label_json=None):
    ...
    with torch.no_grad():   # gradient 계산 메모리 사용을 꺼버림
        ...
        Id_filtered = torch.where(D_arr[np.arange(D_arr.shape[0]),Id_min] < Config.thr_max_dist, Id_min, len(L_ref_arr))
...
def test_color(ckpt_path, label_json=None):
    def load_json(file):
        with open(file, "rt") as f:
            return json.load(f)
